...
use Yeast\Http\Internal\MiddlewareStack;
...
final class Dispatcher
{
    ...
    private function handleRoute(ServerRequestInterface $request, RouteContext $context): ResponseInterface
    {
        ...
        $callable        = null;
        $middlewareStack = $this->module->globalMiddleware;
        ...
        $middlewareStack = array_merge($middlewareStack, $action->middlewares, $context->route->middleware);

        if ($callable === null) {
            throw new RuntimeException("No support for action type of " . get_class($action));
        }

        $middlewareStack = new MiddlewareStack($this->container, $middlewareStack, fn(ServerRequestInterface $modifiedRequest) => $this->handleAction($modifiedRequest, $callable, $context), $this->logger);
    ...
    private function handleAction(ServerRequestInterface $request, array|string $callable, RouteContext $context): mixed
    {
        $action     = $context->action;
        $parameters = [];
